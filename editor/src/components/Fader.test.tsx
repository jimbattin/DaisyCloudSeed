...
describe('Fader', () => {
  it('shows the real value and reports range moves', () => {
    ...
    render(<Fader def={PARAMS['input.PreDelay']} value={0.070000000298023224} preset={lines} badges={['K1B']} onChange={onChange} />);
  ...
  it('reverts out-of-range typed values and commits valid ones', () => {
    const onChange = vi.fn();
    render(<Fader def={PARAMS['input.PreDelay']} value={0.25} preset={lines} badges={[]} onChange={onChange} />);
  ...
  it('enters seeds as the engine integer', () => {
    const onChange = vi.fn();
    render(<Fader def={PARAMS['seeds.TapSeed']} value={0.00115} preset={lines} badges={[]} onChange={onChange} />);
